...
module.exports = new function() {
  ...
  //replace these settings to your own credentials
  var metadata = {
    ...
    amzn_appId: "replace this....", //this is from Alexa Skill
    channelSecretKey: 'replace this....', //This is the secret Key from ODA Channel
    channelUrl: 'replace this....' //This is de Webhook URL from ODA Channel
  };
  // Generate random number to use in case of userID is not avaliable 
  ...
  this.init= function (config) {
    ...
    if (metadata.channelUrl && metadata.channelSecretKey) {
      logger.info('Alexa WebHook - Usa el canal:', metadata.channelUrl);
    }
    ...
    function getChannelSecretKey(req) {
      console.log("Alexa ChannelSecretKey", metadata.channelSecretKey);
        ...
        if (metadata.channelUrl && metadata.channelSecretKey && userId && command) {
          ...
          var sendToAlexa = function (resolve, reject) {
            if (!respondedToAlexa) {
              ...
              logger.info('Preparado para mandar respuesta a alexa');
              resolve();
              PubSub.unsubscribe(userIdTopic);
            } else {
              logger.info("Ya se ha mandado la respuesta");
            }
          };
          ...
          var navigableResponseToAlexa = function (resp) {
            ...
            logger.info("Mensaje a Alexa (Tipo Navegable):", messageToAlexa)
          ...
          var sendMessageToBot = function (messagePayload) {
            ...
            return new Promise(function(resolve, reject){
              var commandResponse = function (msg, data) {
                logger.info('Mensaje recibido por el canal de webhook');
                var resp = data;
                logger.info('Body del mensaje:', resp);
                if (!respondedToAlexa) {
                  navigableResponseToAlexa(resp);
                } else {
                  logger.info("Respuesta ya procesada");
                  return;
                }
              ...
              self.sendWebhookMessageToBot(metadata.channelUrl, metadata.channelSecretKey, userId, messagePayload, additionalProperties, function (err) {
                if (err) {
                  logger.info("Fallo al mandar el mensaje al bot");
                  alexa_res.say("Fallo al mandar el mensaje al bot.  Por favor revisa tu configuración.");
                  reject();
                  PubSub.unsubscribe(userIdTopic);
                }
              });
            ...
            if (menuResponse) {
              ...
              if (['global', 'message'].includes(menu.type)) {
                ...
                if (action.type === 'postback') {
                  ...
                  logger.info('Mandando una ubicación predefinida al bot');
                ...
                if (selectedCard) {
                  ...
                  if (selectedMessage) {
                    //session.set("botMessages", [selectedMessage]);
                      ...
                      logger.info("Mensaje a Alexa (card):", messageToAlexa)
                      alexa_res.say(messageToAlexa);
                ...
                if (returnMessage) {
                  //session.set("botMessages", [returnMessage]);
                    ...
                    _.each(botMessages, function(msg){
                      ...
                      logger.info("Mensaje a Alexa (return card):", messageToAlexa);
                      alexa_res.say(messageToAlexa);
                    })
                    return alexa_res.send();
                }
          ...
          _.defer(function () {
            alexa_res.say("No te entendí. Podrias repertirlo, por favor?");
          });
        }
      }
    );
    ...
    alexa_app.launch(function (alexa_req, alexa_res) {
      ...
      alexa_res.say("Bienvenido a Café Sirena, esta respuesta viene del webhook");
    });
    ...
    alexa_app.pre = function (alexa_req, alexa_res, alexa_type) {
      ...
      if (alexa_req.data.session.application.applicationId != metadata.amzn_appId) {
        ...
        alexa_res.fail("applicationId Invalido");
      }
      logger.info(JSON.stringify(alexa_req.data, null, 4));
      if (!metadata.channelUrl || !metadata.channelSecretKey) {
        var message = "El webhook no puede responder.  Revisa tus configuraciones.";
